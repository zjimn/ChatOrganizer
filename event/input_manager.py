...
from config.constant import TYPE_OPTION_KEY, IMG_SIZE_OPTION_KEY
...
class InputManager:
    ...
    def on_type_option_change(self):
        ...
        if self.input_frame.option_var.get() == self.main_window.options[1]:
            self.show_tree()
            self.app_config.set(TYPE_OPTION_KEY, '1')
        else:
            self.show_tree()
            self.app_config.set(TYPE_OPTION_KEY, '0')
        self.root.event_generate('<<CloseOutputWindowEvent>>')
    ...
    def show_tree(self):
        ...
        self.main_window.display_frame.tree.event_generate('<<UpdateListEvent>>')
    ...
    def on_size_option_change(self):
        ...
        for index, option in enumerate(size_options):
            if self.input_frame.size_var.get() == option:
                self.app_config.set(IMG_SIZE_OPTION_KEY, str(index))
                break
    ...
    def on_hit_submit_button(self):
        if not self.input_frame.submit_button_is_changed:
            ...
            self.root.event_generate('<<CancelRequestEvent>>')
    ...
    def bind_events(self):
        ...
        self.input_frame.submit_button.bind("<<SubmitBackEvent>>", self.show_tree)
